...
impl OrderedLots {
	pub fn new(
		...
		for lot in &mut lots {
			lot.quantity.round(*max_precision_by_currency
				.get(lot.commodity.symbol())
				.expect("Missing symbol for lot precision")
				.min(&max_precision)); // TODO this is heinous
			for sale in &mut lot.sales {
				sale.quantity.round(*max_precision_by_currency
					.get(lot.commodity.symbol())
					.expect("Missing symbol for lot precision")
					.min(&max_precision)); // TODO this is heinous
				if let Some(ref mut proceeds) =
					sale.unit_proceeds
				{
					proceeds.value.round(*max_precision_by_currency
						.get(&proceeds.currency)
						.expect("Missing symbol for lot precision")
						.min(&max_precision)); // TODO this is heinous
				}
			}
